...
describe 'Inn API' do
  ...
  context 'GET /api/v1/inn/1' do
    it 'Exibe uma Pousada' do
      ...
      expect(response.content_type).to include('application/json')
      json_response = JSON.parse(response.body)
      expect(json_response["brand_name"]).to eq('Pousada Boa Vista')
      expect(json_response["brand_name"]).not_to eq('Pousada Árvore da Coruja')
      expect(json_response["brand_name"]).not_to eq('Pousada Nascer do Sol')
    end

    it 'erro 404 caso não encontre uma pousada ativa ou existente' do
      # Arrange
      admin = Admin.create!(name: 'Admin', email: 'admin@admin.com', password: 'password')
      Inn.create!(admin: admin, brand_name: 'Pousada Boa Vista', corporate_name: 'Pousada Good LTDA',
                  registration_number: '94769049000157', phone: '(99)12122-1234', email: 'boavista@email.com.br',
